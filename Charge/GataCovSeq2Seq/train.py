...
def train_epoch(model, data_loader, optimizer, criterion, clip, epoch):
    ...
    for src, tgt in tqdm(data_loader, total=len(data_loader), desc=f"Training {epoch+1}"):
        ...
        # print("output 2: ",output.argmax(axis=1).size())
        # print(type(output), output.data.type())
        # output = output.argmax(axis=1).type(torch.FloatTensor)
        # output = output.type(torch.FloatTensor)
        # print(type(output), output.data.type())
        # print(output.argmax(2).shape)
        tgt = tgt[:,1:].contiguous().view(-1)
        # tgt = tgt.type(torch.FloatTensor)
        # print(type(tgt), tgt.data.type())
        # print("tgt  : ",tgt.size())
